...
Two runs of `/scheduling/` with an identical rig set, well set, financial year and time limit can
return different schedules. The requirements document establishes three independent causes and a
set of secondary ordering hazards. This design closes all of them.
...
(`scheduler/optimization.py:927`, `:954`) makes the amount of search performed depend on how fast
the host happened to run, which is exactly what the reproduction in the requirements measured
(same proto fingerprint `de7ec4d44a8297a2`, three runs, two objective values, deterministic time
drifting 7.1256 → 7.3596). Replacing it with `max_deterministic_time` pins the work performed
(`7.0001` on every run in the same experiment) and makes the answer reproducible whether or not
...
- **Deterministic time**: CP-SAT's machine-independent work counter
  (`CpSolverResponse.deterministic_time`, exposed as `solver.deterministic_time`). Units are
  "as close as possible to a second" per the OR-Tools parameter documentation but are a count of
  work, not elapsed time. `max_deterministic_time` bounds it.
...
- 11 s limit, host under CPU load, identical model proto fingerprint on all three runs: objective
...
1. **The stopping criterion is not a function of the input.** `max_time_in_seconds` at
   `scheduler/optimization.py:927`/`:954` terminates the search at whatever node the host happened
   to reach. The incumbent at that node is the answer. Under CPU contention the reachable node
   varies, which is precisely the measured failure (1 distinct schedule idle, 2 of 3 runs differing
   under load at an 11 s limit).
...
every run: one distinct `schedule_hash`, one distinct `model_fingerprint`, one distinct
`objective_value`, and identical `(well → rig, start day, end day, sequence order)` assignments,
including runs executed while the host is under CPU load.
...
`RATIO` default **0.60**. Derivation from the measurements in the requirements: on the reference
host, an 11 s wall-clock run consumed 7.1–7.4 deterministic units, and a 7.0-unit budget consumed
8.85–8.97 s of wall time — a wall-to-work ratio of ~1.27 s per unit on an idle machine. At
`RATIO = 0.60`, a 300 s selection yields `D = 180`, which costs ≈ 229 s of wall time on the
reference host — 76 % of T, leaving ~1.5× headroom before the `1.15 × T = 345 s` backstop. That
headroom is the point: the run stays deterministic on any host up to ~1.5× slower than the
reference. On a host slower than that the backstop binds, and the run is flagged rather than
...
deployment rather than of the problem. Add to `drilling_scheduler/settings.py` alongside the
existing `VIDEO_PROCESSING` dict precedent (`:227-243`):
...
A slow VM tunes `RATIO` down; a fast VM tunes it up to use more of the user's budget. Tuning it
changes *how much search* is done, which can change the answer — so the value belongs in the
solver fingerprint (decision 7) and must be treated as a schedule-affecting configuration change,
not a performance knob. Document that in the setting comment.

**What "machine-independent" does and does not buy.** Deterministic time removes the dependence on
host speed and load, so the *same amount of search* happens everywhere. That is what makes the
same machine reproducible under load and what makes two machines of different speed agree. It does
not by itself make results identical across *architectures*: CP-SAT's LP layer works in double
precision, so a different CPU family or a differently-compiled wheel can in principle diverge.
The guarantee this design offers is therefore scoped: identical results for a fixed
`(OR-Tools build, platform)`, which covers the deployment target (one wheel, one VM family,
`ortools 9.15.6755` pinned at `requirements.txt:21`). The solver fingerprint makes a build or
parameter change visible as the explanation when it happens, and the on-VM verification command
(decision 8) confirms it on the real host rather than assuming it.
...
derived from a measured wall time. In particular stage 2's backstop is
...
- The two interact with decision 1 in the right direction: LNS is exactly the component whose
  progress varies most with host speed, so putting the total budget on a work counter is what makes
  an LNS-heavy search reproducible.
...
**On-VM verification** — `scheduler/management/commands/check_determinism.py`, alongside the
existing commands (`refresh_ilm_cache.py` etc.):
...
`schedule_hash` appears. This is how the "same selection under heavy CPU load yields the same
hash" and the cross-machine criteria get checked on the real host with real data, which the unit
tests cannot do. It also writes nothing to the database.
